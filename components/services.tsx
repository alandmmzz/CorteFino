      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="flex snap-x snap-mandatory gap-4 overflow-x-auto pb-3 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden md:grid md:grid-cols-2 md:overflow-visible xl:grid-cols-2">
          {catalog.flatMap((service) => service.treatments.map((treatment, index) => (
            <article key={treatment.id} className="flex min-h-56 min-w-[calc(100vw-2.5rem)] snap-start flex-col rounded-2xl border border-foreground/15 bg-foreground/[0.045] p-5 shadow-[0_16px_40px_oklch(0_0_0/0.2)] transition-colors hover:border-primary/60 sm:min-w-[22rem] sm:p-6 md:min-w-0">
