    ...
    <form action={handleSubmit} className="space-y-6">
      ...
      {false ? (
        ...
        <>
          ...
          <button type="submit" disabled={isPending || !selectedDate || !selectedTime || !selectedCategory || selectedTreatments.length === 0} className="w-full rounded-full bg-primary px-8 py-3.5 text-sm tracking-[0.15em] text-primary-foreground transition-opacity hover:opacity-90 disabled:opacity-60">{isPending ? "PROCESANDO..." : DEPOSIT_ENABLED ? "CONTINUAR A LA SEÑA" : "CONFIRMAR RESERVA"}</button>
        </>
      )}
