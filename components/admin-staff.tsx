...
export function AdminStaff({ staff, adminEmails = [] }: { staff: Staff[]; adminEmails?: string[] }) {
  ...
  function submit() {
    startTransition(async () => {
      const result = await createStaff(name, email, adminAccess, photoUrl)
      setMessage(result.ok ? "Personal agregado. Recargá para verlo." : result.error ?? "No se pudo guardar.")
      if (result.ok) {
        setName(""); setEmail(""); setPhotoUrl(""); setAdminAccess(false); setIsModalOpen(false)
      }
    })
  }
      ...
      <div className="mt-6 grid gap-3 sm:grid-cols-2">
        {staff.map((person) => (
          <div key={person.id} className="flex items-center gap-4 rounded-xl border border-border bg-card p-5">
            ...
            <div className="min-w-0"><p className="font-serif text-xl text-foreground">{person.name}</p><p className="mt-1 truncate text-sm text-muted-foreground">{emailForPerson(person) ?? "Email pendiente"}</p><label className="mt-3 flex items-center gap-2 text-sm text-foreground"><input type="checkbox" checked={person.adminAccess || adminEmails.includes(person.email?.toLowerCase().trim() ?? "")} onChange={(event) => toggleAdmin(person.id, event.target.checked)} disabled={!person.email || isPending} />Acceso admin</label></div>
          </div>
        ))}
      </div>
