...
export async function updateAppointmentService(id: number, service: string) {
  let selection: { category?: string; treatmentIds?: (string | number)[] }
...
export async function getBookedTimes(appointmentDate: string, category?: string, staffId?: number) {
  ...
  return (await getAvailableSchedule(category, appointmentDate)).filter((time) => isTimeAvailable(category, time, booked, categoryDuration))
}
